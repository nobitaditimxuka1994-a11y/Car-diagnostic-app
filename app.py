...
# 1. Cấu hình giao diện (Tối ưu cho màn hình dọc điện thoại)
st.set_page_config(page_title="MINH KHANG AUTO", layout="centered")

# Ẩn các thành phần thừa để load nhanh hơn trên 4G
...
def track_search(brand, user_input, found_code, status):
    try:
        # Đọc dữ liệu (ttl=0 để lấy data mới nhất ngay lập tức)
        df = conn.read(ttl=0)
        ...
        pass # Nếu mạng 4G quá yếu không ghi được thì bỏ qua, tránh lỗi 500

# 3. Dữ liệu mã lỗi (Rút gọn để bạn dễ sửa trên điện thoại)
MASTER_DATA = {
    "TOYOTA": {"P0101": "Lỗi MAF: Vệ sinh cảm biến.", "P0171": "Nghèo xăng: Kiểm tra bơm."},
    "VINFAST EV": {"P0A78": "Lỗi Inverter: Kiểm tra làm mát.", "LOI SAC": "Kiểm tra súng sạc."}
}
...
if 'agreed' not in st.session_state:
    if st.button("TÔI ĐỒNG Ý SỬ DỤNG"):
        st.session_state.agreed = True
        st.rerun()
    st.stop()

brand = st.selectbox("Chọn dòng xe:", list(MASTER_DATA.keys()))
user_input = st.text_input("Nhập mã/Hiện tượng:").upper().strip()
...
if user_input:
    ...
    if res:
        st.success(f"🔍 KQ: {res}")
        track_search(brand, user_input, user_input, "Thành công ✅")
    else:
        # Tìm kiếm tương đối nếu không khớp 100%
        found = False
        for k, v in db.items():
            if user_input in k:
                st.info(f"💡 Ý bạn là {k}?\n{v}")
                track_search(brand, user_input, k, "Tìm thấy gần đúng")
                found = True
                break
        if not found:
            st.error("Chưa có dữ liệu.")
            track_search(brand, user_input, None, "Không tìm thấy ❌")

# 5. Xem lịch sử (Chỉ dành cho bạn)
with st.expander("📊 Lịch sử"):
    if st.checkbox("Hiện dữ liệu"):
        try:
